...
public:
   ...
   void PrintInOrder(){///public function
    ...
    void FillVerticalMap(Node * rt, map<int, vector<int> > &m, int level){
        if (rt==NULL) return;
        m[level].push_back(rt->data);
        FillVerticalMap(rt->left,m,level-1);
        FillVerticalMap(rt->right,m,level+1);
    }

    void PrintVertical(){
        cout<<"Vertical layout:"<<endl;
        map<int, vector<int> > m;
        FillVerticalMap(root,m,0);
